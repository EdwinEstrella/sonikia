...
const triggerVariants = cva(
  ["group inline-flex items-center justify-between gap-2 outline-none cursor-pointer",
   ...
   "disabled:opacity-50 disabled:pointer-events-none", "focus-visible:ring-1 focus-visible:ring-purple-500"],
  {
    variants: {
      variant: {
        bordered: "border border-purple-500/20 bg-gray-800/40 text-gray-300 hover:bg-gray-800/60",
        borderless: "border border-transparent bg-transparent text-gray-300 hover:bg-gray-800/60",
      },
    },
...
const SelectTrigger = forwardRef<HTMLButtonElement, SelectTriggerProps>(
  ({ className, variant, placeholder = "Seleccionar…", error, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        >
          ...
          <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className={`shrink-0 text-gray-500 transition-transform duration-80 ${open ? 'rotate-180' : ''}`}>
            <path d="M6 9l6 6 6-6" />
          </svg>
        </button>
        {error && <span className="text-[12px] text-red-400 pl-3">{error}</span>}
      </div>
    );
  }
);
      ...
      <SelectContentContext.Provider value={{ registerItem, activeIndex, checkedIndex }}>
        <div style={{ position: "fixed", top: triggerRect.bottom + 6, left: triggerRect.left, minWidth: triggerRect.width, zIndex: 50 }}>
          <motion.div
            ...
            className={cn(`relative flex flex-col gap-0.5 max-h-[300px] overflow-y-auto ${shape.container} bg-gray-900/95 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-purple-500/20 p-1 select-none outline-none`, className)}
          ...
          >
            <AnimatePresence>
              {checkedRect && (
                <motion.div className={`absolute ${shape.bg} bg-purple-500/20 pointer-events-none`} initial={false}
            ...
            <AnimatePresence>
              {activeRect && (
                <motion.div key={sessionRef.current} className={`absolute ${shape.bg} bg-pink-500/15 pointer-events-none`}
            ...
            <AnimatePresence>
              {focusRect && (
                <motion.div className={`absolute ${shape.focusRing} pointer-events-none z-20 border border-purple-500`} initial={false}
...
const SelectItem = forwardRef<HTMLDivElement, SelectItemProps>(
  ({ className, children, value, index, disabled = false, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn(`relative z-10 flex items-center gap-2 ${shape.item} px-2 py-2 text-[13px] cursor-pointer outline-none select-none`, "transition-[color] duration-80", isActive || isChecked ? "text-white" : "text-gray-400", disabled && "opacity-50 pointer-events-none", "hover:bg-purple-500/10", className)}
      {...props}
    >
      <span className="flex-1 min-w-0 truncate">{children}</span>
      <AnimatePresence>
        {isChecked && (
          <motion.svg key="check" width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-purple-400" initial={{ opacity: 1 }} animate={{ opacity: 1 }} exit={{ opacity: 1 }}>
...
const SelectLabel = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => <div ref={ref} className={cn("px-2 py-1.5 text-[11px] text-gray-500", className)} {...props} />
);
SelectLabel.displayName = "SelectLabel";

const SelectSeparator = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => <div ref={ref} role="separator" className={cn("my-1 -mx-1 h-px bg-purple-500/20", className)} {...props} />
);
